## This section gives the temp;late of g=how to check if a 
## multicollinearlity exists between the different variables
...
# This is nice sincer you get a nicer picture
corrplot(descrCor)
...
#Dropping response variable (Y) for calculating Multicollinearity
mat_b = subset(data3, select = -c(medv))

# we only want numerical variables for computing correlation
numeric <- mat_b[sapply(mat_b, is.numeric)]

#Calculating Correlation- strength of association between two variables
descrCorr <- cor(numeric)
print(descrCorr)
